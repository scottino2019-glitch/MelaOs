  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({
        ...
        message: "Chiave API Gemini assente. Configura la chiave GEMINI_API_KEY nelle impostazioni / variabili d'ambiente di Vercel per abilitare Siri."
      });
    }
    ...
    console.error("Vercel Siri API Handler error:", error);
    return res.status(500).json({ 
      error: "SIRI_API_FAIL", 
      message: error?.message || "Errore del server Siri." 
    });
  }
}
